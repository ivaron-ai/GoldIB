...
$JobDir = if ($env:JOB_DIR) { $env:JOB_DIR } else { Join-Path (Split-Path $PSScriptRoot -Parent) (Join-Path "ralph-jobs" $JobName) }
...
$RepoRootForGit = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
...
while ($iteration -lt $maxIteration) {
    ...
    if ($GitAvailable) {
        ...
        $hasChanges = (Invoke-Git @('diff', '--cached', '--quiet')) 2>&1
